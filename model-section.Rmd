...
```{r pic-asymptotic-corr, fig.align = "center", fig.cap = "Long-run squared correlation between genetics $x_1$ and SES $x_2$ (heritability of SES), by weight of genetics in spouse matching ($a$) and strength of direct transmission of SES ($\\theta$). $\\tau$ = 0.95.", dpi = 300, out.width="6in", dev = "cairo_pdf"}

theta <- seq(0, 0.95, 0.025)
a <- seq(0, 0.99, 0.015)
h2 <- outer(theta, a, FUN = Vectorize(heritability))
opar <- par(mar = rep(1, 4))
persp(theta, a , h2, theta = 300, phi = 25,
      xlim = c(0, 1), ylim = c(0, 1),
      xlab = "θ", # theta
      zlab = "h²",
      border = "darkgreen", col = "grey75", ltheta = 90, lphi = 35,
...
```{r pic-simulate-mao, fig.cap = "A simulated policy shock under GAM ($a$ = 1) and SGAM ($a$ = 0.3). Lines show mean SES in each quartile of families, grouped by SES in the generation before the shock. In generation 0 , $\\theta$ and $\\gamma$ are reduced to zero. Thereafter they return to their previous values ($\\theta = 0.8, \\gamma = 0.1$). Under GAM, family SES returns to the no-shock counterfactual (dashed lines). Under SGAM it never does so.", cache=FALSE}
...
simulate_shock <- function (n, a, tau, theta, gamma, n_gens_pre, 
                            n_gens_shock, n_gens_post,
                            theta_shock = theta, gamma_shock = gamma) {
  # we keep populations identical before the shock:
  set.seed(317548932)
  x_1 <- rnorm(n)
  x_2 <- rnorm(n)
  male <- rep(c(FALSE, TRUE), n/2)
  male_line_id <- 1:n
  pop <- data.frame(x_1 = x_1, x_2 = x_2, male = male, 
                    male_line_id = male_line_id)
  
  stats <- pop[0, ] # zero row data to start
  stats$gen <- numeric(0)
  update_stats <- function(stats, pop) {
    pop$gen <- if (length(stats$gen)) {max(stats$gen) + 1} else 1
    rbind(stats, pop)
  }
  stats <- update_stats(stats, pop)
  
  # get to equilibrium, we hope
  # first generation is before matching, hence the minus 1
  ...
  stats$state <- rep(c("pre", "shock", "post"), 
                     c(n_gens_pre, n_gens_shock, n_gens_post))
...
params <- tidyr::crossing(
                          ...
                          n_gens_shock = 1,
                          n_gens_post = 50) |> 
  filter(sign(theta_shock) == sign(gamma_shock))

res <- pmap(params, simulate_shock)
res <- list_rbind(res)
res <- cbind(params[rep(1:4, each = 10000*151), ], res)
res$shock <- res$theta_shock == 0
res$model <- ifelse(res$a == 1, "GAM", "SGAM")

gen100 <- res |> filter(gen == 100) |> 
  summarize(.by = c(model, shock, male_line_id),
    x_1 = mean(x_1),
    x_2 = mean(x_2)
  )
res <- left_join(res, 
                       gen100 |> select(model, shock, male_line_id, x_1, x_2),
                       by = c("model", "shock", "male_line_id"),
                       suffix = c("", "_gen_100"))
res |> 
  filter(male == TRUE) |> 
  mutate(.by = c(model, shock),
    qtile_gen_100 = cut(x_2_gen_100, 4, labels = 1:4)
  ) |> 
  mutate(.by = c(model, shock, gen),
            x_1_norm = c(scale(x_1)),
            x_2_norm = c(scale(x_2)),
            ) |> 
  summarize(.by = c(model, shock, gen, qtile_gen_100),
            mean_x_1 = mean(x_1_norm),
            mean_x_2 = mean(x_2_norm)
            ) |> 
  filter(gen %in% 95:110) |> 
  mutate(
    Generation = gen - 100 - 1,
    `Gen 100 quartile` = qtile_gen_100,
    `Gen 100 quartile` = ifelse(shock == FALSE, NA, `Gen 100 quartile`)
  ) |> 
  ggplot(aes(Generation, mean_x_2, group = interaction(qtile_gen_100, shock),
             colour = `Gen 100 quartile`, linetype = shock)) + 
    geom_point(data = function (x) x[x$shock,], 
               aes(shape = factor(`Gen 100 quartile`)), size = 0.9) +  
    geom_line() + 
    
    facet_wrap(vars(model), axes = "all") +
    ...
    labs(y = "Mean SES") + 
    scale_shape_manual(values = c("4" = "circle", "3" = "square", 
                                  "2" = "diamond", "1" = "triangle")) +
    scale_color_viridis_c(option = "B", na.value = "grey50",
...
Gale-Shapley matching algorithm shows that the basic insights of SGAM still hold.[^gale-shapley] 

[^gale-shapley]: See https://github.com/hughjonesd/trading-status-genetics/blob/main/gale-shapley-multiple-a.R.
...
raised by @young1958rise and more recently @markovits2019meritocracy: meritocracy 
