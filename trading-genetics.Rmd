...
author: |
  ...
  Pierre-André Chiappori\thanks{Department of Economics, Columbia University, 
...
abstract: |
  ...
  more education. We find evidence that this effect is mediated by individuals' own education and 
...
# load moba results
# we load the revised results first, because we want the old 4-column
# version of export_table3dummy_tidyse; the old version overwrites just
# that variable.
#load("moba-results/tradinggenetics2026_v1.Rdata")
#load("moba-results/tradinggenetics_moba_v04.Rdata")
load("moba-results/tradinggenetics2026_v02.Rdata")
#load("moba-results/tradinggenetics_moba_revised_2026_v02.Rdata")
#load("moba-results/tradinggenetics_moba_sibs_revised_2026_v02.Rdata")
...
*genes-SES gradients* -- systematic genetic differences between people
with high and low SES [@belsky2018genetic; @Rimfeld_2018;
...
@solon2018we]. One possible explanation for this is that couples match
on an underlying variable, which SES is a noisy measure for.
@clark2023inheritance argues that the data mostly fits a model where
...
measures of educational attainment [@okbay2022polygenic]. To isolate the
causal link from own SES to partner's genes, we use a shock to SES which
is independent of own genetics. Specifically, we use a person's *birth
...
attainment and occupational status [@black2011older; @booth2009birth;
@Lindahl_2008]. At the same time, the facts of biology, in particular
the so-called "lottery of meiosis", make it likely that siblings' birth
order is independent of their genetic endowments.[^2] Because birth
order could affect partner choice through both SES and non-SES
mechanisms, we run a mediation analysis similar to
@heckman2013understanding: we estimate a reduced-form model with spouse
...
[^2]: In meiosis, each child's DNA is drawn randomly and independently
    ...
    assumption: we test for this below. @muslimova2020dynamic find that
    PSEA and birth order *interact* to produce human capital.
...
genetically inherited traits. @halsey1958genetics showed that social
...
inherited social status. @cloninger1979multifactorial model genetic and
...
to be associated in offspring. @heath1985resolving, following earlier
...
Most recently @rustichini2023educational model intergenerational
...
individuals, is around 50% [@Tambs_1989]. The family socio-economic
status of 2-year-old children can be predicted from their genes
[@Trzaskowski_2014]. Children born into higher-income families have more
genetic variants predicting educational attainment [@belsky2018genetic].
...
transmission of wealth and human capital [e.g. @bjorklund2006origins].
...
with regional economic deprivation [@abdellaoui2019genetic]. The
correlation between education and health may be mediated by shared
genetic causes [@amin2015schooling; @boardman2015can]. Family SES
correlates with several health-related polygenic scores
[@selzam2019comparing], and genetic variants associated with SES may
...
[@belsky2018genetic].[^4] This mechanism depends on meritocractic social
institutions [@branigan2013variation; @Heath_1985]: in a society where
...
returns to talent [@Rimfeld_2018]. By contrast, SGAM does not require
...
[^4]: @belsky2018genetic offer three reasons for the association between
    education-linked genetic variants and SES, but do not mention SGAM.
...
predicted from parent-child correlations [@clark2015intergenerational;
@barone2021intergenerational; @solon2018we], and distant relatives in
the same generation are also more similar than parent-child and spousal
correlations would predict [@collado2023estimating].
@clark2023inheritance argues that unobserved genetic variation
...
example, @alesina2020persistence show that elite households lost their
...
As @okbay2022polygenic put it: "Because the parameters of a general
...
to estimate other parameters." (See also @sunde2025understanding.) For
example, @collado2023estimating derive extremely low estimates for
...
phenotype, or on a shared cultural factor; @torvik2022modeling estimate
...
between siblings but not across generations; while @clark2023inheritance
...
effect, we can reject $a = 1$.[^5] In other words, we set out to test
...
[^5]: Conceivably, if $a = 0$ but there is a pre-existing correlation
...
occasion;[^6] both reported the same homeownership/renting status,
...
`r pretty(nrow(mf_pairs))` pairs.[^7]

[^6]: A typical UK postcode contains about 15 properties.

[^7]: In the appendix, we test the validity of our matching process by
...
@papageorge2020genes].[^8]

[^8]: See @papageorge2020genes for a detailed discussion of polygenic
...
children will also have higher PSEA on average.[^9]

[^9]: Figure \@ref(fig:pic-basic-corr-moba) in the appendix shows the
...
observed education phenotype alone [@okbay2022polygenic]. To demonstrate
...
@shakespeare1595midsummer, the spouse matching process is highly
...
[@Lindahl_2008; @booth2009birth; @black2011older].[^10] On the other
...
supported by the biological mechanism of meiosis, which ensures that at
a given autosomal locus, each parent transmits one of their two alleles
with 50% probability [@mendel1865experiments; @lawlor2008mendelian]. For
example, siblings' expected polygenic score is equal to the mean of
their parents' polygenic scores. We can therefore use birth order as a
"shock" to social status. "Shock" is in quotes because we do not claim
that birth order is exogenous to all other variables. For example, it
naturally correlates with parental age, and it may also correlate with
household SES at the time of birth. We only claim that birth order is
exogenous to genetic variation.

[^10]: Earlier work was ambiguous on the effects of birth order [e.g.
...
effects from parental age [@cochran2013paternal], though recent research
has rejected these in favour of “social” explanations
[@kristensen2007explaining; @black2011older]. To control for family
...
differ by birth order.[^11] We assume differences in unmeasured
investments due to $b$ are independent of $\mathbf{X}$.

[^11]: Under the assumption that measured and unmeasured mediators are
...
By running a least squares regression of \@ref(eq:model-to-estimate), we
...
OLS regressions based on equation \@ref(eq:mediator-model) for each
...
\@ref(eq:model-to-estimate), multiplied by the coefficient of birth
order on university/income from equation \@ref(eq:mediator-model). We
...
parental age from birth order.[^12] This reduces the N by a lot, since
...
[^12]: A possible critique is that later siblings by definition have
...
which would bias our results towards zero.[^13]

[^13]: Table \@ref(tab:tbl-bo-psea-no-par-age) in the appendix reports
...
"\\ref{tab:tbl-bo-psea}, columns 2 to 4, explained by each mediating variable.")
...
regression: the coefficient of birth order is about -0.03 (and very
...
"EA3".[^14] Income is from all sources, reported at age 30, converted to
...
continuing education or in relationships already.[^15] Data on IQ and
...
[^14]: The $R^2$ on own university attendance is
    `r pretty(r2_own_psea_moba, 3)` for EA4 in MoBa, compared to
    `r pretty(r2_own_psea, 3)` for EA3 in UK Biobank.

[^15]: We also tried income at age 25. This gave odd results, with a
...
males and females.[^16] As in Britain, coefficients look very similar
...
[^16]: We don't run separate regressions for families with children,
...
# columns 2 and 6 added a "scl" variable as a robustness check
# this is from the revised version 2026
mod_bo_psea_moba <- purrr::map2(export_table3_tidyse[c(1, 3:5)],
                                  export_table3_glance[c(1, 3:5)],
...
mediator_effect_bo <- purrr::map_dbl(export_table1_tidyse[1:4], 
                                     function (x) {
                                       x$estimate[x$term == "parity"]
                                     })

mediators <- c("university", "incomez", "height", "bmi")
psea_effect_mediators <- purrr::map(export_table3_tidyse[3:5],
                                        function (x){
...
tbl_note <- paste("Percentage of the effects of birth order in Table", 
"\\ref{tab:tbl-bo-psea-moba}, columns 2 to 4, explained by each mediating variable.")
...
moba_subset_cols <- c("column1male", "column4male",
                      "column1female", "column4female")

mod_bo_subsets_moba <- purrr::map2(
  export_table5_tidyse[moba_subset_cols],
  export_table5_glance[moba_subset_cols],
  convert_moba_for_huxreg)
...
tbl_note <- paste("Estimates from OLS regressions corresponding to",
"columns 1 and 4 in Table \\ref{{tab:tbl-bo-psea-moba}}, separately",
...
sample of siblings from the MoBa data[^17], and regress spouse's
...
spouse SES could be *mediated* by own SES, cf. the extended model of
meritocracy in equation (\ref{eqn-gamma})).

[^17]: The UK Biobank sample has too few siblings with spouses for this
    analysis to be informative.

```{r tbl-reversed-moba}

tidy_uni_psea_moba <- sibresults_2023_tidyse[[2]]
glance_uni_psea_moba <- sibresults_2023_glance[[2]]
tidy_uni_psea_moba <- convert_moba_for_huxreg(tidy_uni_psea_moba,
                                              glance_uni_psea_moba)

tidy_inc_psea_moba <- sibresults_2023_tidyse[[6]]
glance_inc_psea_moba <- sibresults_2023_glance[[6]]
...
n_within_checks_sig <- sum(balance_check_within$p.value < 0.05)
...
```{r calc-placebo-moba}
balance_moba <- dplyr::bind_rows(balancedescriptives_tidyse, .id = "dep_var") |> 
  filter(grepl("self", dep_var)) 

n_sig_moba <- sum(balance_moba$p.value < 0.05)
n_tests_moba <- nrow(balance_moba)
n_pgis_moba <- n_distinct(balance_moba$dep_var)

stopifnot(n_sig_moba/n_tests_moba < 0.05)
```
...
a full set of family size dummies. Of the `r n_within_coefs` birth order
coefficients, `r n_within_checks_sig` were significant at $p < 0.05$;
...
PSEA (effect size `r pretty(coef_own_psea_3, 3)`,
...
on MoBa data. In regressions of `r n_pgis_moba` polygenic scores on
birth order dummies, `r n_sig_moba` out of `r n_tests_moba` coefficients
...
is indeed exogenous to genetics (as @isungset2022birth also find).[^18]

[^18]: UKBB polygenic scores were residualized on the first 100
    ...
    construction, see @abdellaoui2019genetic. MoBa polygenic scores were
...
noise [@young2022mendelian]. Third, it is hard to imagine an assortative
...
social status.[^19] It may be possible to directly test for genetic
differences across social status in ancient DNA samples.

[^19]: The appendix has a selection of relevant historical quotations.
...
<https://github.com/hughjonesd/trading-status-genetics>.
...
In UK Biobank, we calculate the maximum $R^2$ as `r R2max`.[^20] For
...
$R^2$ as `r R2max_moba`.[^21] For university attendance, $\delta^*$ is
...
[^20]: In other regressions of spouse traits on PSEA, $R^2$ are indeed
    below this value [@hugh2016assortative; @okbay2022polygenic].

[^21]: The MoBa number is larger because the $R^2$ of EA4 on education
...
mod_bo_1st_moba <- purrr::map2(export_table1_tidyse[1:4],
                                     export_table1_glance[1:4], 
                                     convert_moba_for_huxreg)
...
```{r tbl-bo-psea-fix-sample}

mf_pairs_reg_no_nas <- mf_pairs_reg %>%
  filter(
    ! is.na(EA3.x),
...
mod_bo_fix_sample <- lapply(fml_bo_psea_base, fixest::feols,
                     data  = mf_pairs_reg_no_nas, 
                     notes = FALSE
                   )


mf_pairs_reg_income <- mf_pairs_reg %>%
  filter(
    ! is.na(first_job_pay.x)
  )

mod_bo_income_sample <- lapply(fml_bo_psea, fixest::feols,
                     data  = mf_pairs_reg_income, 
                     notes = FALSE
                   )
# we could put this in response to reviewers
# huxreg(mod_bo_income_sample)
...
"ages), and further controls (family size, birth year, and birth month dummies)", 
"in columns 2-3 to ensure the balance of covariates",
"across birth order. Sample is kept constant across specifications. 
All data is from the UK Biobank for a sample of UK",
"individuals born between 1935 and 1970.", my_note)

huxreg(mod_bo_fix_sample, 
         coefs = c(
           "Birth order"           = "birth_order.x", 
           "Own PSEA"              = "EA3.x",
           "Parents' age at birth" = "par_age_birth.x"
       ...
       set_font_size(10) %>%
       set_escape_contents(12, 1, FALSE) %>%
       set_caption("Regressions of spouse PSEA on birth order, holding sample constant (Great Britain)")
...
```{r tbl-bo-psea-no-yob}


fml_bo_no_yob <- lapply(fml_bo_psea, update, 
                     . ~ . | . - factor(YOB.x) - factor(birth_mon.x)
                   )
mod_bo_no_yob <- lapply(fml_bo_no_yob, fixest::feols,
                     data  = mf_pairs_reg, 
                     notes = FALSE
                   )

huxreg(mod_bo_no_yob, 
         coefs     = reg_coefs[! reg_coefs %in% c("factor(YOB.x)",
                                                  "factor(birth_mon.x)")],
         note      = my_note,
         stars     = my_stars,
         tidy_args = tidy_args
       ...
       insert_row(after = 19, "Family size dummies", rep("Yes", 4)) %>% 
       insert_row(after = 20, "Birth month dummies", rep("No", 4)) %>% 
       insert_row(after = 21, "Birth year dummies", rep("No", 4)) %>% 
       set_bottom_border(20:21, everywhere, 0) %>% 
       set_align(20:21, -1, "center") %>% 
       set_number_format(2:19, -1, 4) %>% 
       set_font_size(11) %>%
       set_caption("Regressions of spouse PSEA, without controls for respondents'
                   year or month of birth (Great Britain)")


```

```{r tbl-bo-psea-no-yob-moba, eval = FALSE}

# the first column doesn't have controls for height/bmi so the
# difference with the other columns will include the effect of these.
# leave it unless we're asked.

mod_bo_no_yob_moba <- purrr::map2(export_table3_nobirthadjust_tidyse,
                                       export_table3_nobirthadjust_glance, 
                                       convert_moba_for_huxreg)

# columns 2 and 6 added scl
huxreg(unname(mod_bo_no_yob_moba[c(1, 3:5)]), 
       coefs     = moba_reg_coefs,
       note      = my_note,
       stars     = my_stars,
       tidy_args = tidy_args
  ...
  insert_row(after = 15, "Family size dummies", rep("Yes", 4)) %>%
  insert_row(after = 16, "Birth month dummies", rep("No", 4)) %>%
  insert_row(after = 17, "Birth year dummies", rep("No", 4)) %>%
  set_bottom_border(16:17, everywhere, 0) %>%
  set_align(16:17, -1, "center") %>%
  set_number_format(2:15, -1, 4) %>%
  set_font_size(11) %>%
  set_caption("Regressions of spouse PSEA, without controls for respondents'
                   year or month of birth (Norway)")

```

```{r tbl-bo-psea-only-educ}
fml_bo_only_educ <- lapply(fml_bo_psea, update, 
                     . ~ . - fluid_iq.x - height.x - bmi.x - health.x, 
                     rhs = 1
                   )
mod_bo_only_educ <- lapply(fml_bo_only_educ, fixest::feols,
                     data  = mf_pairs_reg, 
                     notes = FALSE
                   )

coefs_only_educ <- reg_coefs[c(1:3, 8:9)]

huxreg(mod_bo_only_educ, 
         coefs     = coefs_only_educ,
         note      = my_note,
         stars     = my_stars,
         tidy_args = tidy_args
       ) %>% 
       ...
       set_font_size(11) %>%
       set_caption("Regressions of spouse PSEA, without controls for height, BMI, IQ or health (Great Britain)")
...
# columns 2 and 6 added scl
mod_bo_psea_moba_dummies <- purrr::map2(export_table3dummy_tidyse[c(1, 3:5)],
                                  export_table3dummy_glance[c(1, 3:5)],
                                  convert_moba_for_huxreg)
...
```{r pic-bo-psea-interactions-moba, fig.align = "center", fig.cap = "Regressions of spouse PSEA: birth order dummies within different family sizes (Norway). Labels show birth order. Lines are 95 per cent confidence intervals. The omitted category is birth order 1."}

famsize_moba <- bind_rows(famsizestratified_tidyse, .id = "famsize")
famsize_moba |> 
  filter(grepl("parity", term)) |> 
  mutate(
    `Family size` = as.numeric(sub("famsize", "", famsize)),
    `Birth order` = as.numeric(sub("factor(parity)", "", term, fixed = TRUE)),
    conf.low = estimate - 1.96 * std.error,
    conf.high = estimate + 1.96 * std.error,
  ) |> 
  ggplot(aes(estimate, `Family size`, colour = `Birth order`)) + 
    scale_color_continuous(type = "viridis", name = "Birth order") + 
    geom_vline(xintercept = 0, linetype = 2) +
    geom_pointrange(aes(xmin = conf.low, xmax = conf.high), 
                    alpha = 0.7, size = 1.15, fatten = 1,
...
mod_bo_psea_age_fte_moba <- purrr::map2(export_table3eduyears_tidyse[c(1, 3, 5)],
                                  export_table3eduyears_glance[c(1, 3, 5)],
                                  convert_moba_for_huxreg)
