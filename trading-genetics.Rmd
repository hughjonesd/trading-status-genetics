...
$x_2$, and reject the null of no effect, we can reject $a = 1$.[^5] In
...
is equal to the mean of their parents' polygenic scores.[^11] We can
...
[^11]: Although genetic variation is randomly assigned to children at
...
@black2011older]. @cochran2013paternal report that mutational load is
...
differ by birth order.[^12] We assume differences in unmeasured
investments due to $b$ are independent of $\mathbf{X}$.

[^12]: Under the assumption that measured and unmeasured mediators are
...
parental age from birth order.[^13] This reduces the N by a lot, since
...
[^13]: A possible critique is that later siblings by definition have
...
which would bias our results towards zero.[^14]

[^14]: Table \@ref(tab:tbl-bo-psea-no-par-age) in the appendix reports
...
"EA3".[^15] Income is from all sources, reported at age 30, converted to
...
continuing education or in relationships already.[^16] Data on IQ and
...
[^15]: The $R^2$ on own university attendance is
    `r pretty(r2_own_psea_moba, 3)` for EA4 in MoBa, compared to
    `r pretty(r2_own_psea, 3)` for EA3 in UK Biobank.

[^16]: We also tried income at age 25. This gave odd results, with a
...
males and females.[^17] As in Britain, coefficients look very similar
...
[^17]: We don't run separate regressions for families with children,
...
sample of siblings from the MoBa data[^18], and regress spouse's
...
[^18]: The UK Biobank sample has too few siblings with spouses for this
...
different polygenic scores in the UK Biobank sample.[^19] We regress
...
[^19]: Polygenic scores were residualized on the first 100 principal
...
social status.[^20] In future, it may be possible to directly test for
genetic differences across social status in ancient DNA samples.

[^20]: The appendix has a selection of relevant historical quotations.
...
In UK Biobank, we calculate the maximum $R^2$ as `r R2max`.[^21] For
...
$R^2$ as `r R2max_moba`.[^22] For university attendance, $\delta^*$ is
...
[^21]: In other regressions of spouse traits on PSEA, $R^2$ are indeed
    below this value [@hugh2016assortative; @okbay2022polygenic].

[^22]: The MoBa number is larger because the $R^2$ of EA4 on education
