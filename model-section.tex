...
{\centering \includegraphics[width=6in]{model-section_files/figure-latex/pic-asymptotic-corr-1.-rgl} 
...
Using this extension, we can examine how intergenerational transmission
of inequality responds to policy shocks under different models of assortative
mating. Figure (\textbf{ref?})(fig:pic-simulate-mao) simulates different generations of
...
reduces the mating value of previously high-SES individuals. As a result, their
SES never recovers to its no-shock value.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{model-section_files/figure-latex/pic-simulate-mao-1.pdf}}
\caption{\label{fig:pic-simulate-mao}A simulated policy shock under GAM (\(a\) = 1) and SGAM (\(a\) = 0.3). Lines show mean SES in each quartile of families, grouped by SES in the generation before the shock. In generation 0 , \(\theta\) and \(\gamma\) are reduced to zero. Thereafter they return to their previous values. Under GAM, family SES swiftly returns to the no-shock counterfactual (dashed lines). Under SGAM it never does so.}
\end{figure}
